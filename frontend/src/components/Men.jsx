...
const Men = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await axios.get(`${serverurl}/api/products/all?category=Men`)
        setProducts(res.data)
      } catch (error) {
        console.error("Fetch men products error", error)
      }
    }
    fetchProducts()
  }, [serverurl])
    ...
    <div className='w-full min-h-screen bg-[#050505] text-white pt-10 pb-20 px-4 md:px-12'>
      ...
      <div className='max-w-[1400px] mx-auto mb-12 text-center'>
        ...
        <p className='text-gray-400 max-w-lg mx-auto text-sm md:text-base'>
          Refined essentials for the modern man. Discover the latest trends in tailoring, casual wear, and accessories.
        </p>
      </div>
