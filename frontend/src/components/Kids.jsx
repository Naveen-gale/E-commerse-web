...
const Kids = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const res = await axios.get(`${serverurl}/api/products/all?category=Kids`)
        setProducts(res.data)
      } catch (error) {
        console.error("Fetch kids error", error)
      }
    }
    fetch()
  }, [serverurl])
    ...
    <div className='w-full min-h-screen bg-[#050505] text-white pt-10 pb-20 px-4 md:px-12'>
      <div className='max-w-[1400px] mx-auto mb-12 text-center'>
        ...
        <p className='text-gray-400 max-w-lg mx-auto text-sm md:text-base'>
          Playful styles for little explorers. Durable, comfortable, and trendy outfits for every adventure.
        </p>
      </div>
      ...
      <div className='max-w-[1400px] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-y-12 gap-x-8'>
        {products.length > 0 ? (
          ...
          <div className="col-span-full text-center text-gray-500 py-20">
            No products found in Kids' category.
          </div>
        )}
      </div>
